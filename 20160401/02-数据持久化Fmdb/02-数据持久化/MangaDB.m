...
{
    BOOL result = [self.dbase executeUpdateWithFormat:@"update students set name = %@, age = %ld,phone = %@, icon = %@ where ID = %ld",mode.name,mode.age,mode.phone,mode.data,mode.ID];
    if (!result) {
        NSLog(@"更新失败");
    }
       return result;
...
-(NSMutableArray *)selectAllStudent{
    FMResultSet *set= [self.dbase executeQueryWithFormat:@"select * from students"];
...
-(BOOL)deleteFromStudent:(int)ID{
    BOOL result = [self.dbase executeUpdateWithFormat:@"delete from students where ID = %d",ID];
    if (!result) {
        NSLog(@" 删除失败");
    }
    return result;
